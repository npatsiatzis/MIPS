...
covered_XY = []
g_data_width = 4	#should be set to 32, but in my machine it gives memory error
...
@CoverPoint("top.c",xf = lambda dut : dut.o_result.value, bins = list(range(2**(g_data_width+1)-1)), at_least=1)
...
def adder_randomised_test(dut):
	...
	while(full != True):
		...
		yield Timer(2)
		io_cover(dut)
		assert not (adder_model(A,B) != dut.o_result.value)
	...
	coverage_db.export_to_xml(filename="coverage_adder32.xml")